...
async def monitor_loop():
    ...
    while True:
        try:
            ...
            print(f"   (aguardando {CHECK_INTERVAL_MINUTES} minutos...)\n", flush=True)
            ...
            print(f"\n✗ Erro no loop: {e}", flush=True)
            import traceback
            traceback.print_exc()
            print("⏳ Aguardando 5 minutos antes de continuar...", flush=True)
            await asyncio.sleep(300)

if __name__ == "__main__":
    try:
        if not validate_config():
            print("\n❌ Finalizando devido a erro de configuração", flush=True)
            sys.exit(1)
        
        print("\n✓ Configuração validada com sucesso!", flush=True)
